...
- **배포 후 발견된 빌드 함정 (server-only × fat barrel) — 2단계로 해소**: `passport-crypto.ts`의 초기 `import "server-only"`(부수효과 전용 import라 트리셰이킹에서 제거되지 않음)가 `@/entities/booking` 배럴을 통해 'use client' 컴포넌트(CancelBookingButton·CheckoutForm)의 클라 번들로 끌려가 `node:crypto` Unhandled scheme 으로 `npm run build` 실패. `typecheck`/`vitest`(server-only를 mock alias)는 이 경로를 못 잡는다 — **server-only/배럴 경계 변경 시 반드시 `npm run build` 로 검증할 것.**
  - **(1차) client-safe 서브배럴 분리** — booking 슬라이스에 `@/entities/booking/client.ts`(순수 model/constants/schemas/transitions/progress만, `./api/*` 제외)를 신설하고 두 클라 컴포넌트를 거기로 재지정. `index.ts`(server 포함 fat barrel)는 server importer 전용. 향후 'use client' 에서 booking import 시 반드시 `/client` 사용. (user 슬라이스는 `import type` 만 클라에서 쓰여 erase 되므로 분리 불요.) **이 분리가 누수 차단의 load-bearing 수정.**
  - **(2차) crypto 에서 `server-only` 제거** — 운영 백필 `scripts/encrypt-passports.ts` 를 bare `tsx`(Node)로 돌리면 `server-only`(Next/webpack·vitest 에서만 해석)가 `Cannot find module 'server-only'` 로 죽는다. 이는 `db.ts`/`env.ts` 가 server 전용임에도 `server-only` 를 안 붙이는 **기존 프로젝트 컨벤션과 동일한 이유**(tsx 스크립트·배럴 호환). 1차 분리로 클라 누수가 이미 막혔으므로 server-only 는 불필요했고(제거 후에도 `npm run build` exit 0·981 테스트 통과 확인), 누수 차단은 client.ts 분리 + "잘못 import 시 env 가 브라우저에서 ZodError" 트립와이어(db/env 동형)로 유지.
  - 더 근본적으론 모든 슬라이스의 `index.ts`=client-safe / `server.ts`=server 전면 분리가 이상적이나 영향 범위가 커 후속 과제로 남김.
