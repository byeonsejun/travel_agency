...
### Amendment (2026-06-13): 회귀 케이스 사후 진단 → 트리거·프롬프트 동결(음성결과)

가중치 튜닝이 한계수익(ADR-0049)에 도달한 뒤 "다음 레버"로 **유일 하락 케이스(`일상에서 벗어나고 싶다` −0.258)의 미세조정**이 거론됐다. 조정 전 15쿼리 실패모드를 진단한 결과 **조정 근거 없음**으로 결론, 트리거·프롬프트를 **현 상태로 동결**한다.

**진단 결과(15쿼리 per-query 분류):** WIN 10 · NO-OP 4 · LOSS 1.

- **유일 LOSS는 라벨 주관성이 지배.** `일상에서 벗어나고 싶다`의 rerank 1위는 다낭(라벨 **2**, 적합 답)이고, 페널티는 거의 전부 **몰디브(3)↔다낭(2)의 2-vs-3 한 끗**에서 발생. 동일한 "몰디브 매장" 패턴인데도 `북적이지 않는 곳`(+0.302)·`특별한 날 기념`(+0.380)은 크게 개선 → **체계적 reranker 실패모드가 아니라 단일 어노테이션 주관성**.
- **희소 라벨 함정.** 쿼리당 4~5개만 라벨링·나머지 코퍼스는 자동 0이라 "상위3 내 라벨0"은 *부적합*이 아니라 *미판정*을 센다. WIN 다수가 동일 falseTop3 값 → 이 지표로 reranker를 단죄 불가.
- **NO-OP 중 `활기차게 즐기고 싶어`는 hybrid도 1위가 라벨0** = reranker가 아닌 **하이브리드/코퍼스 커버리지 공백**(좋은 답이 top-8 부재) → 재정렬이 고칠 수 없는 영역.

**왜 조정하지 않는가:**
1. **한계수익 함정 재현 방지.** 15쿼리·희소·초안 라벨에서 단일 주관 케이스를 잡으려 프롬프트를 비트는 것은 ADR-0049가 가중치에서 멈춘 바로 그 한계수익 곡선의 가장 가파른 끝을 rerank 프롬프트로 옮겨오는 것.
2. **동결 픽스처 순환모순.** `rerank.fixture.json`은 *현 프롬프트가 생성한* 스냅샷 → 프롬프트 조정은 기존 픽스처로 측정 불가(구프롬프트 출력으로 신프롬프트 채점). 정직한 재측정은 키 필요한 픽스처 재추출을 강제 → 키 0 결정론 eval(축 3)의 핵심 가치 훼손.

**결론:** mean nDCG@5 **+0.1915는 실측·견고**(10승 1패, 패는 라벨노이즈). 트리거·프롬프트 동결이 정답. **더 밀어붙일 다음 레버는 프롬프트가 아니라 라벨 밀도/품질 향상**(쿼리당 판정 8~10개로 측정 도구를 먼저 날카롭게) — 현 데이터로는 조정 근거가 없다는 것이 정확한 표현. (진단 수치는 `run-eval.ts --rerank`로 결정론 재현.)
