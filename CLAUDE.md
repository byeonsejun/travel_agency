...
- **다음 작업자의 혼란 방지 노트 (Phase 8)**:
  ...
  - "부분환불 후 메일이 왜 안 오나?" → [ADR-0036] Notes. PARTIAL_REFUND_COMPLETED 미구현. 현재는 기존 REFUND_COMPLETED 경로만 커버. 별도 에픽 대상.
...
- **다음 작업자의 혼란 방지 노트**:
  ...
  - "대시보드 Recharts 차트에 `db`를 import하면?" → (Phase 6) [ADR-0033] **금지**. 차트는 `window`/`ResizeObserver` 의존이라 `'use client'` 리프 2개(`RevenueTrendChart`/`BookingStatusDonut`)에만 격리하고, 서버가 집계한 plain 배열을 props로 주입. KPI 카드·기간 필터·조립(`AdminDashboard`)은 전부 server. 경계 회귀는 `grep "use client" src/widgets/admin-dashboard/ui/`로 2개만 나오는지 1초 검증. 기간 필터는 `useState` 아닌 `<Link href="?range=">`(searchParams SSOT).
