...
- **현재는 UI 전면 개편(A1 클린 블루) 완료 / 다음 마일스톤 미정** — 디자인 시스템 기반(shadcn 프리미티브 8종 `src/shared/ui/{button,card,input,tabs,badge,select,dropdown-menu,sheet}.tsx` + `cn()` `src/shared/lib/utils.ts` + 토큰) 구축 완료. `(admin)` 셸은 아직 레거시 스타일(이번 개편은 `(site)` 한정). `ReviewStatus.REPORTED` enum은 여전히 예약 유지(미사용). 다음 후보: admin 셸 A1 적용, 검색 가중치 튜닝(golden-query/nDCG eval), 태그 vocabulary 동기화, 추가 알림 채널(SMS/카카오), 출발취소 cascade 위약금 옵션, ADR 발행(REPORTED status-flip 포기 결정 박제).
