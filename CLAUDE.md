...
- **현재 마일스톤: "일관성 → 문서화 → 알고리즘" 3단계 로드맵 (확정, 2026-06-10)** — 디자인 시스템 기반(shadcn 프리미티브 8종 `src/shared/ui/{button,card,input,tabs,badge,select,dropdown-menu,sheet}.tsx` + `cn()` `src/shared/lib/utils.ts` + 토큰) 구축 완료. 다음 순서로 진행:
  ...
  3. **[다음] 검색 가중치 튜닝 (데이터 기반 고도화)** — golden-query 셋 + nDCG eval 하네스로 벡터(0.5)/키워드(0.2)/geo(0.2)/테마(0.1) 밸런스 정량 검증.
  - 백로그(이 3단계 이후): 추가 알림 채널(SMS/카카오), 출발취소 cascade 위약금 옵션.
- **다음 작업자의 혼란 방지 노트 (Phase 5-B)**:
  ...
  - "revalidateTag에 왜 `'max'`가 붙었나?" → 16 시그니처 강제(2-arg). `'max'`=stale-while-revalidate, 즉시성은 같은 액션의 revalidatePath가 보완. updateTag 채택은 Phase 5-C.
  ...
  - "lint에서 react-hooks 규칙이 일부 꺼져 있다?" → [ADR-0052]. react-hooks@7 신규 규칙 14개만 parity 위해 비활성(classic `rules-of-hooks`/`exhaustive-deps`는 활성 유지). 재활성화는 Phase 5-C 별도 정리.
