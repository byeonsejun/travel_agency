...
- **현재 마일스톤: "일관성 → 문서화 → 알고리즘" 3단계 로드맵 (확정, 2026-06-10)** — 디자인 시스템 기반(shadcn 프리미티브 8종 `src/shared/ui/{button,card,input,tabs,badge,select,dropdown-menu,sheet}.tsx` + `cn()` `src/shared/lib/utils.ts` + 토큰) 구축 완료. 다음 순서로 진행:
  1. **[진행 중] Admin 셸 A1 적용** — `(admin)` 셸이 아직 레거시 스타일(직전 UI 개편은 `(site)` 한정). `shared/ui` 프리미티브 8종을 재사용해 A1 클린 블루로 통일. 데이터 밀도 높은 admin 테이블·폼에 테마 변형 적용.
  2. **ADR 발행 (문서화 부채 청산)** — `ReviewStatus.REPORTED` enum을 status-flip에 안 쓰고 report-driven 큐로 간 결정 박제(REPORTED status-flip 포기). enum은 여전히 예약 유지(미사용).
  3. **검색 가중치 튜닝 (데이터 기반 고도화)** — golden-query 셋 + nDCG eval 하네스로 벡터(0.5)/키워드(0.2)/geo(0.2)/테마(0.1) 밸런스 정량 검증.
