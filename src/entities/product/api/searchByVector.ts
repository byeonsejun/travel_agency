...
function buildFilterClauses(filters: VectorSearchFilters): Prisma.Sql[] {
  ...
  if (d?.max !== undefined) {
    clauses.push(Prisma.sql`AND p."durationNights" <= ${d.max}`);
  }
...
async function keywordFallback(
  ...
  const rows = await db.$queryRaw<ScoredRow[]>(Prisma.sql`
    ...
    WHERE p.status = 'PUBLISHED'
      AND (p.title ILIKE ${like} OR p.destination ILIKE ${like} OR p.summary ILIKE ${like})
...
export async function searchProductsByVector(
  ...
  try {
    if (!(await isPgvectorAvailable())) {
      return await keywordFallback(keywordText, filters);
    }
    ...
    const rows = await db.$queryRaw<ScoredRow[]>(Prisma.sql`
      SELECT p.id AS id, 1 - (e.vector <=> ${vecLiteral}::vector) AS score
      FROM "Product" p
